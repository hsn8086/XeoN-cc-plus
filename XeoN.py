...
def checking(lines, proxy_type, ms, rlock, ):  # Proxy checker coded by XeoN_K
    global nums
    global proxies
...
def check_socks(ms):  # Coded by XeoN_K
    global nums
    thread_list = []
...
def get_proxy():
    ...
    while len(proxies) == 0:
        try:
            r = requests.get('http://127.0.0.1:5010/all')
            json_str = r.text
        except:
            json_str = '{}'
        l = json.loads(json_str)

        proxies = [i['proxy'] for i in l]


if __name__ == "__main__":
    main()  # Coded by XeoN_K
